...
function AdminTiendas() {
  ...
  try {
    ...
    if (!respuesta.ok) {
      await mostrarError(
        respuesta,
        "No se pudieron cargar los emprendedores."
      );
      setEmprendedores([]);
      return;
    }
  ...
  try {
    ...
    if (!respuesta.ok) {
      await mostrarError(respuesta, "No se pudo crear la tienda. Revisa los datos.");
      return;
    }
  ...
  try {
    ...
    if (!respuesta.ok) {
      await mostrarError(
        respuesta,
        "No se pudo actualizar la tienda. Revisa los datos."
      );
      return;
    }
  ...
  try {
    const respuesta = await fetch(`${API_TIENDAS}/${id}`, {
      method: "DELETE",
      headers: headersAuth(),
    });

    if (!respuesta.ok) {
      await mostrarError(respuesta, "No se pudo eliminar la tienda.");
      return;
    }
